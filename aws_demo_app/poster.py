...
class KinesisPoster(threading.Thread):
    def __init__(self, stream_name, partition_key, poster_time=30, quiet=False,
        ...
        self.default_records = ['hello', 'how are you?', 'goodbye', 'another message']
        self.quiet = quiet
        ...
        if filename is not None:
            ...
            with open(filename, 'r') as content_file:
                self.field_contents = content_file.read(40000)
    ...
    def put_file_contents(self):
        if self.file_contents:
            response = kinesis.put_record(
                ...
                data=self.file_contents, partition_key=self.patition_key
            )
    ...
    def run(self):
        ...
        while finish > datetime.datetime.now():
            if self.file_contents:
                ...
                rand_num_str = str(random.randint(0,5000))
                rand_msg = self.default_records[random.randint(0, 3)]
                self.add_records(rand_msg)
                # self.add_records([rand_msg, rand_num_str, rand_msg + rand_num_str])
